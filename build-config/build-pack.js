...
*/
const options = {
  appId: 'com.visoncube.music.desktop',
  productName: 'VisonCube Music',
  npmRebuild: process.env.VISONCUBE_OFFLINE_BUILD !== '1',
  buildDependenciesFromSource: false,
  beforePack,
  afterPack,
  protocols: {
    name: 'visoncube-music-protocol',
    schemes: [
      'visoncubemusic',
      'lxmusic',
    ],
...
const linuxOptions = {
  linux: {
      maintainer: 'VisonCube Music <maintainer@visoncube.invalid>',
    ...
    desktop: {
      ...
      entry: {
        Name: 'VisonCube Music',
        'Name[zh_CN]': 'VisonCube Music',
        'Name[zh_TW]': 'VisonCube Music',
        Encoding: 'UTF-8',
        MimeType: 'x-scheme-handler/visoncubemusic;x-scheme-handler/lxmusic',
        StartupNotify: 'false',
      },
    },
  },
...
const macOptions = {
  ...
  dmg: {
    ...
    title: 'VisonCube Music v${version}',
  },
}
...
const createTarget = {
  ...
  linux(arch, packageType) {
    switch (packageType) {
      case 'deb':
        linuxOptions.artifactName = `VisonCube-Music_\${version}_${arch == 'x64' ? 'amd64' : arch}.\${ext}`
        return {
          buildOptions: { linux: ['deb'] },
          options: linuxOptions,
        }
      case 'appImage':
        linuxOptions.artifactName = `VisonCube-Music_\${version}_${arch}.\${ext}`
        return {
      ...
      case 'rpm':
        linuxOptions.artifactName = `VisonCube-Music-\${version}.${arch}.\${ext}`
        return {
  ...
  mac(arch, packageType) {
    switch (packageType) {
      case 'dmg':
        macOptions.artifactName = `VisonCube-Music-\${version}-${arch}.\${ext}`
        return {
...
const build = async(target, arch, packageType, publishType) => {
  if (target == 'dir') {
    await builder.build({
      dir: true,
      config: { ...options, ...winOptions, ...linuxOptions, ...macOptions, electronDist: './node_modules/electron/dist' },
    })
    return
  }
  const targetInfo = createTarget[target](arch, packageType)
  const buildConfig = { ...options, ...targetInfo.options }
  if (target == 'win') buildConfig.electronDist = './node_modules/electron/dist'
  // Promise is returned
  await builder.build({
